...
### 1.7 สร้าง Firestore Composite Indexes (สำคัญมาก!)

Firestore ต้องสร้าง Composite Index สำหรับ Query ที่ใช้หลาย field

1. Firebase Console → Firestore Database → แท็บ **Indexes** → **Composite**
2. กด **Create index** → สร้างทีละตัว:

| # | Collection | Field 1 | Field 2 | หน้าที่ |
|---|---|---|---|---|
| 1 | `keys` | `productId` ↑ | `claimed` ↑ | ค้นหาคีย์ตามสินค้า |
| 2 | `keys` | `claimed` ↑ | `claimedBy` ↑ | คีย์ที่ผู้ใช้กดรับ |
| 3 | `topUpHistory` | `userId` ↑ | `createdAt` ↓ | ประวัติเติมเงินเรียงวันที่ |
| 4 | `walletTransactions` | `userId` ↑ | `createdAt` ↓ | ธุรกรรมกระเป๋าเงิน |
| 5 | `boosterOrders` | `userId` ↑ | `createdAt` ↓ | ประวัติสั่งซื้อ Booster |
| 6 | `claimHistory` | `userId` ↑ | `claimedAt` ↓ | ประวัติกดรับคีย์ |

หรือ Deploy ผ่าน Firebase CLI ด้วยไฟล์ `firestore.indexes.json`:
...
> ⚠️ **ถ้าไม่สร้าง Indexes → Query ที่ใช้หลาย field จะ Error** `requires an index` **ทันที!** แต่ละ Index ใช้เวลาสร้างประมาณ 1-5 นาที
...
| Webhook Booster สั่งซื้อ | แจ้งเมื่อมีออเดอร์ Booster ใหม่ |

---

## 5. Booster Service — บริการเพิ่มยอด Social (ไม่บังคับ)

### 5.1 สมัครบัญชี Provider

1. เข้าเว็บ Provider (เช่น MeeLike หรือ SMM Panel อื่นๆ)
2. สมัครสมาชิก → เติมเงินเข้าบัญชี
3. คัดลอก **API Key** จาก Dashboard/Settings

> 💡 ยอดเงินใน Provider = ต้นทุน เมื่อลูกค้าสั่งซื้อ → หักจากบัญชี Provider อัตโนมัติ

### 5.2 ตั้งค่า MEELIKE_API_KEY

ไปที่ Supabase Dashboard → **Project Settings** → **Edge Functions** → **Secrets**

| Secret Name | ค่า | ใช้โดย |
|---|---|---|
| `MEELIKE_API_KEY` | API Key จาก Provider | meelike-proxy (Edge Function) |

> ⚠️ ถ้าไม่ตั้ง `MEELIKE_API_KEY` → บริการ Booster จะใช้งานไม่ได้!

### 5.3 ตั้งค่า Booster ในหน้า Admin

Admin → ตั้งค่าเว็บไซต์ → หมวด **Booster**:

| การตั้งค่า | คำอธิบาย |
|---|---|
| เปิด/ปิดบริการ | สวิตช์เปิด-ปิดระบบทั้งหมด |
| โหมดปิดปรับปรุง | แสดงข้อความ Maintenance |
| ชื่อแบรนด์ | ชื่อที่แสดงแทน Provider จริง |
| ข้อความแจ้งเตือน | Banner แจ้งผู้ใช้ |
| จำนวนขั้นต่ำ/สูงสุด | จำกัดปริมาณต่อออเดอร์ |
| คืนเครดิตอัตโนมัติ | คืนเงินเมื่อ API Error |
| แจ้งเตือนยอดต่ำ | เกณฑ์แจ้งเตือนยอด Provider เหลือน้อย |
| จำกัดยศ | เลือกยศที่เข้าถึงบริการได้ |

### 5.4 ตั้งค่า Markup กำไร

ไปที่ Booster Admin Panel (`/boosteradminpanel`):

- **Global Markup** — กำไร % ที่ใช้กับทุกบริการ (เริ่มต้น 20%)
- **Category Markup** — กำไร % แยกตามหมวดหมู่ (override Global)

ตัวอย่าง: ราคาต้นทุน ฿1.00/1K → Markup 30% → ราคาขาย ฿1.30/1K → กำไร ฿0.30

---

## 6. Deploy & เริ่มใช้งาน
...
### 6.4 ตั้งค่าเบื้องต้นในหน้า Admin
...
- [ ] (ถ้าใช้ Booster) สั่งซื้อบริการ Booster สำเร็จ
- [ ] (ถ้าใช้ Booster) ประวัติออเดอร์แสดงถูกต้อง
- [ ] (ถ้าใช้ Booster) Webhook Booster แจ้งเตือน
...
| `boosterOrders` | ประวัติสั่งซื้อบริการ Booster |
